...
# اسکریپت نصب برای سرور ایران (کلاینت)
# این اسکریپت فقط برای سرور ایران طراحی شده است
...
show_banner() {
    ...
    echo "║                    نصب سرور ایران v1.0                   ║"
    echo "╚══════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}

# تابع دریافت اطلاعات سرور خارج
get_foreign_server_info() {
    ...
    echo -n -e "${BLUE}IP سرور خارج: ${NC}"
    read -r FOREIGN_IP
    
    echo -n -e "${BLUE}پورت SSH سرور خارج [2222]: ${NC}"
    read -r FOREIGN_PORT
    FOREIGN_PORT=${FOREIGN_PORT:-2222}
    
    echo -n -e "${BLUE}نام کاربری SSH [tunnel]: ${NC}"
    read -r FOREIGN_USER
    FOREIGN_USER=${FOREIGN_USER:-tunnel}
    
    echo -n -e "${BLUE}پورت محلی تانل [8080]: ${NC}"
    read -r LOCAL_PORT
    LOCAL_PORT=${LOCAL_PORT:-8080}
    
    echo -n -e "${BLUE}پورت تانل روی سرور خارج [1080]: ${NC}"
    read -r TUNNEL_PORT
...
# Tunnel Configuration - Iran Server (Client)
# Created: $(date)
...
EOF
    ...
    log_message "تنظیمات تانل برای سرور ایران ایجاد شد"
}
...
# Tunnel Management Script - Iran Server (Client)
case "${1:-help}" in
    "start")
        echo "شروع تانل کلاینت..."
        ...
        ;;
    "stop")
        echo "توقف تانل کلاینت..."
        if [[ -f /etc/tunnel/config.conf ]]; then
            /opt/tunnel/tunnel_client.sh stop
        fi
        ;;
    "status")
        echo "وضعیت تانل کلاینت:"
        if [[ -f /etc/tunnel/config.conf ]]; then
            source /etc/tunnel/config.conf
            echo "نوع سرور: $SERVER_TYPE (کلاینت)"
        ...
        ;;
    "monitor")
        echo "مانیتورینگ زنده تانل کلاینت..."
        /opt/tunnel/tunnel_manager.sh monitor
        ;;
    "optimize")
        echo "بهینه‌سازی تانل کلاینت..."
    ...
    "restart")
        ...
        ;;
    "help"|*)
        echo "استفاده از Tunnel (سرور ایران):"
        echo "  tunnel start      # شروع تانل کلاینت"
        echo "  tunnel stop       # توقف تانل کلاینت"
        echo "  tunnel restart    # راه‌اندازی مجدد تانل"
        echo "  tunnel status     # وضعیت تانل کلاینت"
...
show_install_summary() {
    echo -e "${GREEN}=== نصب سرور ایران با موفقیت تکمیل شد ===${NC}"
    echo ""
    ...
    echo ""
    echo -e "${BLUE}🌐 تنظیمات سرور ایران:${NC}"
    echo "  - نوع سرور: کلاینت (ایران)"
    echo "  - IP خارج: $FOREIGN_IP"
    ...
    echo ""
    echo -e "${YELLOW}دستورات تانل کلاینت:${NC}"
    echo "  tunnel start              # شروع تانل کلاینت"
    echo "  tunnel stop               # توقف تانل کلاینت"
    echo "  tunnel status             # وضعیت تانل کلاینت"
    echo "  tunnel monitor            # مانیتورینگ زنده"
