...
detect_server_type() {
    ...
    echo ""
    echo -e "${YELLOW}لطفاً نوع سرور خود را انتخاب کنید:${NC}"
    echo ""
    echo -e "${GREEN}1)${NC} سرور ایران (کلاینت) - اتصال به سرور خارج"
    echo -e "${GREEN}2)${NC} سرور خارج (سرور) - دریافت اتصال از سرور ایران"
    echo ""
    echo -e "${YELLOW}⚠️  مهم: لطفاً عدد 1 یا 2 را تایپ کنید و Enter بزنید${NC}"
    echo -e "${YELLOW}💡 اگر نمی‌توانید تایپ کنید، Ctrl+C را فشار دهید و دوباره اجرا کنید${NC}"
    echo ""
    
    # بررسی اینکه آیا stdin در دسترس است
    if [[ ! -t 0 ]]; then
        echo -e "${RED}❌ ورودی در دسترس نیست. استفاده از تنظیمات پیش‌فرض: سرور خارج${NC}"
        SERVER_TYPE="foreign"
        return
    fi
    
    while true; do
        echo -n -e "${BLUE}نوع سرور شما (1 یا 2): ${NC}"
        read -r choice
        case $choice in
            1)
                SERVER_TYPE="iran"
...
get_server_info() {
    if [[ "$SERVER_TYPE" == "iran" ]]; then
        ...
        echo ""
        echo -e "${YELLOW}تنظیمات پیش‌فرض برای سرور خارج:${NC}"
